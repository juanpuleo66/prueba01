...
@Component({
    ...
    providers: [LoginService, ConfigsService]
})

export class ConfigDuplicateComponent implements OnInit{
	public titulo: string = 'Duplicate Config';
...
//console.warn('F-1:config.duplicate.componenet.ts-onSumbmit');	
		...
		if ( cont ) {
			...
			this._configsService.duplicate(token, this.config, dataConfig, this.itemsPerPage).subscribe(
				response => {
					this.status = response.status;
